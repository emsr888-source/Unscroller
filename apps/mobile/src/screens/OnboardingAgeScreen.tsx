...
export default function OnboardingAgeScreen({ navigation }: Props) {
  ...
  const { ageRangeLabel, setAgeRangeLabel } = useOnboardingAssessment();
  const [selectedAge, setSelectedAge] = useState<string | null>(
    AGE_OPTIONS.find(option => option.label === ageRangeLabel)?.id ?? null,
  );
  ...
  const handleSelectAge = (ageId: string) => {
    ...
    const label = AGE_OPTIONS.find(option => option.id === ageId)?.label ?? null;
    setAgeRangeLabel(label);
    navigation.navigate('OnboardingBuildFocus');
  };
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
        <View style={[styles.page, { paddingTop: topPadding }]}> 
          ...
          >
            ...
            <Text style={styles.subtitle}>Pick the range that fits best.</Text>

            <View style={styles.options}>
              {AGE_OPTIONS.map((option) => (
                <WatercolorButton
                  key={option.id}
                  color={selectedAge === option.id ? 'yellow' : 'neutral'}
                  onPress={() => handleSelectAge(option.id)}
                  style={styles.optionButton}
                >
                  <Text style={[styles.optionText, selectedAge === option.id && styles.optionTextSelected]}>
                    {option.label}
                  </Text>
                </WatercolorButton>
              ))}
